...
# --- Process Uploaded PDF (with OCR fallback) ---
def process_pdf(upload):
    ...
    docs = []
    try:
        # Try extracting text with PyPDFLoader
        loader = PyPDFLoader(path)
        docs = loader.load()

        # If all pages are empty → trigger OCR
        if all(doc.page_content.strip() == "" for doc in docs):
            raise ValueError("Empty PDF text detected")
    except:
        # OCR fallback
        st.sidebar.warning("PDF seems scanned. Running OCR...")
        images = convert_from_path(path)
        for i, image in enumerate(images):
            text = pytesseract.image_to_string(image, lang="eng")
            docs.append(type("Doc", (), {"page_content": text, "metadata": {"page": i}})())
...
# --- Title ---
st.title("Chhguat with Your PDF (with OCR support)")

# --- Initialize Chain ---
if upload_file and st.sesssion_state.qa_chain is None:
